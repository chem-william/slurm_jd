...
**slurm_jd** is licenced under the MIT license, and so are all
contributions. Please see the [`LICENSE`] file in this directory for more details.

[`LICENSE`]: https://github.com/chem-william/slurm_jd/blob/main/LICENSE

## Pull Requests

To make changes to **slurm_jd**, please send in pull requests on GitHub to
...
GitHub](https://github.com/chem-william/slurm_jd/issues). If you've found a
problem with **slurm_jd**, please open an issue there.

<!--
We use the following labels:

* `enhancement`: This is for any request for new sections or functionality.
* `bug`: This is for anything that's in `slurm_jd`, but incorrect or not working.
* `discussion`: A discussion about improving something in `slurm_jd`; this may
...
Report bugs at https://github.com/chem-william/slurm_jd/issues.
...
`slurm_jd` could always use more documentation, whether as part of the
official `slurm_jd` docs, in docstrings, or even on the web in blog posts,
articles, and such.

### Submit Feedback

The best way to send feedback is to file an issue at https://github.com/chem-william/slurm_jd/issues.
...
Ready to contribute? Here's how to set up `slurm_jd` for local development.

1. Fork the `slurm_jd` repo on GitHub.
2. Clone your fork locally:
    ```shell
    $ git clone git@github.com:your_name_here/slurm_jd.git
    ```
